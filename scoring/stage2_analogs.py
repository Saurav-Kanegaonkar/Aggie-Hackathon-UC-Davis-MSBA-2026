...
def enrich_recovery_analogs(df: pd.DataFrame, panel: pd.DataFrame) -> pd.DataFrame:
    ...
    out["recovery_analog_eins"] = None
    out["recovery_analog_count"] = 0
    out["recovery_analog_evidence"] = None
    ...
    for (constraint_label, metric_name), indices in constraint_groups.items():
        ...
        if pool.empty:
            for idx in indices:
                ...
                out.at[idx, "recovery_analog_eins"] = json.dumps([])
                out.at[idx, "recovery_analog_evidence"] = json.dumps([])
            continue
        ...
        for idx in indices:
            ...
            if len(selected) == 0:
                ...
                out.at[idx, "recovery_analog_eins"] = json.dumps(eins)
                out.at[idx, "recovery_analog_evidence"] = json.dumps(evidence)
