...
def enrich_urgency_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Contract: not_scoreable rows → urgency_flag = false regardless of raw values
    not_scoreable = out.get("benchmark_status", pd.Series(dtype="object")) == "not_scoreable"

    # Binary urgency rule: shock_absorption_months < 3 AND operating_margin < 0
    has_both = shock_abs.notna() & margin.notna()
    urgent = has_both & (shock_abs < 3) & (margin < 0) & ~not_scoreable
