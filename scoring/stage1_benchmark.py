...
def _find_resilient_peers(
    ...
    # No step produced enough — return whatever the last step gave (or empty)
    return qualifying_eins, len(benchmark_fallback_order), benchmark_fallback_order[-1]["label"]
...
def build_benchmarks(scored_df, full_panel, contract):
    ...
    for (ck, fy), group_indices in groups.groups.items():
        ...
        if ref_count >= min_reference_orgs:
            ...
            bm_status = "insufficient_resilient_refs"
        else:
            bm_status = "insufficient_resilient_refs"
    ...
    def _key_for_row(row):
        for level_idx, level_cols in enumerate(fallback_order):
            ...
            if size >= min_cohort_size:
                ...
                for c in level_cols:
                    v = row.get(c)
                    parts.append(f"{c}=unclassified" if pd.isna(v) else f"{c}={v}")
                return "|".join(parts)
