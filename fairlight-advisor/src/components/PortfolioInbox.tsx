...
import type { OrganizationRecord, StrategyLane } from "../types";
...
const TAB_CONFIGS: Array<{
  ...
  label: string;
  description: string;
  actionLabel?: OrganizationRecord["actionLabel"];
}> = [
  {
    id: "uab",
...
export function PortfolioInbox({
  ...
  laneFilter: _laneFilter,
  lanesAvailable: _lanesAvailable,
  layoutMode,
  onActionFilterChange: _onActionFilterChange,
  onLaneFilterChange: _onLaneFilterChange,
  onSearchQueryChange,
  ...
  actionFilter: string;
  laneFilter: StrategyLane | "All";
  lanesAvailable: boolean;
  layoutMode: "gallery" | "rail";
  onActionFilterChange: (value: string) => void;
  onLaneFilterChange: (value: StrategyLane | "All") => void;
  ...
  const tabFilteredOrganizations = useMemo(() => {
    if (bucketTab === "uab") {
      return organizations.filter((o) => o.actionLabel === "Underinvested Asset Base");
    }
    if (bucketTab === "rcr") {
      return organizations.filter((o) => o.actionLabel === "Revenue Concentration Risk");
    }
    if (allCasesChip === "wff") {
      return organizations.filter((o) => o.actionLabel === "Weak Financial Foundation");
    }
    if (allCasesChip === "ndd") {
      return organizations.filter((o) => o.actionLabel === "Needs Data Diligence");
    }
  ...
  const displayedOrganizations = isGallery
    ...
    : [
        ...tabFilteredOrganizations.filter((o) => o.id === selectedId),
        ...tabFilteredOrganizations.filter((o) => o.id !== selectedId).slice(0, 5),
      ];
  ...
  const sizeOptions = useMemo(
    () => REVENUE_BUCKET_ORDER.filter((v) => v === "All" || sizeBucketOptions.includes(v)),
    [sizeBucketOptions],
  );
  const stateFilterOptions = useMemo(() => ["All", ...stateOptions], [stateOptions]);

  const uabCount = organizations.filter((o) => o.actionLabel === "Underinvested Asset Base").length;
  const rcrCount = organizations.filter((o) => o.actionLabel === "Revenue Concentration Risk").length;

  return (
    <section
      className="w-full rounded-[2.8rem] border border-black/6 bg-[rgba(255,253,248,0.72)] p-2 shadow-[0_34px_94px_-56px_rgba(15,23,42,0.28)]"
    >
      <div className="rounded-[calc(2.8rem-0.5rem)] bg-[linear-gradient(180deg,rgba(255,255,255,0.94),rgba(251,248,242,0.86))] px-5 pb-5 pt-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] sm:px-6">
        ...
        <div className="space-y-5 border-b border-black/6 pb-5">
          ...
          {/* Three-tab navigation — always visible */}
          <div className="flex flex-wrap gap-2">
            {TAB_CONFIGS.map((tab) => {
              const count =
                tab.id === "uab" ? uabCount : tab.id === "rcr" ? rcrCount : organizations.length;
          ...
          >
            ...
            {bucketTab === "all" ? (
              <DropdownPill
                ...
                onToggle={() => setOpenToolbarKey((c) => (c === "bucket-filter" ? null : "bucket-filter"))}
                onSelect={(v) => {
                  setAllCasesChip(v as AllCasesChip);
                  setOpenToolbarKey(null);
                }}
                options={[
                  { value: "all", label: "All" },
            ...
            <DropdownPill
              ...
              onToggle={() =>
                setOpenToolbarKey((c) => (c === "size-bucket" ? null : "size-bucket"))
              }
              onSelect={(v) => {
                onSizeBucketFilterChange(v);
                setOpenToolbarKey(null);
              }}
              options={sizeOptions.map((v) => ({ value: v, label: revenueBucketLabel(v) }))}
            />
            ...
            <DropdownPill
              ...
              onToggle={() => setOpenToolbarKey((c) => (c === "sort" ? null : "sort"))}
              onSelect={(v) => {
                onSortOptionChange(v as SortOption);
                setOpenToolbarKey(null);
              }}
            ...
            <DropdownPill
              ...
              onToggle={() => setOpenToolbarKey((c) => (c === "state" ? null : "state"))}
              onSelect={(v) => {
                onStateFilterChange(v);
                setOpenToolbarKey(null);
              }}
              options={stateFilterOptions.map((v) => ({ value: v, label: v }))}
            />
          </div>
